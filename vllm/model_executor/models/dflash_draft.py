...
from .qwen2 import Qwen2MLP as DFlashMLP
from .utils import (
    AutoWeightsLoader,
...
class DFlashDraftAttention(nn.Module):
    """DFlash draft attention matching shisa_dflash v2 exports.

    Context K/V tensors are pre-inserted into the KV cache. The forward path
    only processes query tokens, using the same Q/K/V/O projection weights that
    the offline trainer exports.
    """
...
class DFlashDraftDecoderLayer(nn.Module):
    def __init__(
        self,
        ...
        *,
        ...
        self.self_attn = DFlashDraftAttention(
            ...
            cache_config=cache_config,
            quant_config=quant_config,
            rope_parameters=_get_dflash_rope_parameters(config),
...
class DFlashDraftModel(nn.Module):
    ...
    def _build_fused_kv_buffers(self) -> None:
        ...
        for attn in layers_attn[1:]:
            ...
            ), "All layers must have the same RoPE parameters for DFlash precompute"
        ...
        self._rms_norm_eps = self.hidden_norm.variance_epsilon
        for attn in layers_attn[1:]:
            assert (
                attn.kv_size == self._kv_size
                and attn.head_dim == self._head_dim
                and attn.num_kv_heads == self._num_kv_heads
            ), "All layers must have the same attn config for DFlash precompute"
...
class DFlashDraftForCausalLM(nn.Module):
